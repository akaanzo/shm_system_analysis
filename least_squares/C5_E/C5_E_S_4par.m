...
dataPurged = [tpurged, spurged, T1purged, T2purged];
...
k = 1;
for i = 1 : n
    if(isfinite(spurged(i)) && isfinite(T1purged(i)) && isfinite(T2purged(i)))
            dt(k, 1) = tpurged(i) - tpurged(1);
            ds(k, 1) = spurged(i) - spurged(1);
            dT1(k, 1) = T1purged(i) - T1purged(1);
            dT2(k, 1) = T2purged(i) - T2purged(1);
            k = k+1;
    end
end
len = length(dt)
...
D = [ones(size(dtnd)), dtnd, dT1nd, dT2nd];
...
subplot(2,1,2)
hold on
plot(tpurged(1:nd),(spurged(1:nd)-spurged(1))-ETHETA(3,1)*(T1purged(1:nd)-T1purged(1))-ETHETA(4,1)*(T2purged(1:nd)-T2purged(1)),'-b');

plot(tpurged(1:nd),mymodel_4par([tpurged(1:nd)-tpurged(1)  zeros(length(tpurged(1:nd)))],ETHETA),'-r'); % zeros instead temperature data
plot(tpurged(1:nd),mymodel_4par([tpurged(1:nd)-tpurged(1)  zeros(length(tpurged(1:nd)))],ETHETA+[0; sqrt(SIGMA(2,2)); 0; 0]),':r'); % error estimation above
plot(tpurged(1:nd),mymodel_4par([tpurged(1:nd)-tpurged(1)  zeros(length(tpurged(1:nd)))],ETHETA-[0; sqrt(SIGMA(2,2)); 0; 0]),':r'); % error estimation below
